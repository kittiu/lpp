...
function update_scrap_items(frm) {
    ...
    if (production_item) {
        frappe.call({
            ...
            args: {
                'doctype': 'Item',
                'fieldname': 'custom_item_group_2',  // Field you want to get the value from
                'filters': {'name': production_item}  // Filter to find the right doc
                },
            callback: function(r) {
                if (r.message.custom_item_group_2) {
                    ...
                    frm.doc.scrap_items.forEach(row => {
                        ...
                        row.custom_item_group_2 = r.message.custom_item_group_2;
...
function set_scrap_item_code(frm, cdt, cdn) {
    ...
    if (production_item) {
        // First, make the API call to fetch the item group
        frappe.call({
            method: 'frappe.client.get_value',
            args: {
                'doctype': 'Item',
            ...
            callback: function(r) {
                if (r.message.custom_item_group_2) {
                    ...
                    frappe.model.set_value(cdt, cdn, 'custom_item_group_2', r.message.custom_item_group_2);
                } else {
                    frappe.model.set_value(cdt, cdn, 'item_code', production_item);
                    frappe.model.set_value(cdt, cdn, 'item_name', production_item_name);
                }
