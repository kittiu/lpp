...
class ReceivablePayableReport:
	...
	def run(self, args):
		...
		self.get_data()
		# self.get_chart_data()
		return self.columns, self.data, None, self.skip_total_row
	...
	def set_ageing(self, row):
		...
		if getdate(entry_date) > getdate(self.filters.report_date):
			row.range1 = row.range2 = row.range3 = row.range4 = row.range5 = row.range6 = row.range7 = 0.0

		row.total_due = row.range1 + row.range2 + row.range3 + row.range4 + row.range5 + row.range6 + row.range7

	def get_ageing_data(self, entry_date, row):
		# [0-15,15-30,30-45, -60, 60-90, 90-120, 120-above]
		row.range1 = row.range2 = row.range3 = row.range4 = row.range5 = row.range6 = row.range7 = 0.0
		...
		if not (self.filters.range1 and self.filters.range2 and self.filters.range3 and self.filters.range4 and self.filters.range5 and self.filters.range6):
			self.filters.range1, self.filters.range2, self.filters.range3, self.filters.range4, self.filters.range5, self.filters.range6 = (
				15,
				30,
				45,
				60,
	...
	def get_columns(self):
		...
		self.add_column(label=_("Balance"), fieldname="total_due", fieldtype="Currency")
		...
		for i, label in enumerate(
			[
				...
				"{range4}-{range5}".format(
					range4=cint(self.filters["range4"]) + 1, range5=self.filters["range5"]
				),
				"{range5}-{range6}".format(
					range5=cint(self.filters["range5"]) + 1, range6=self.filters["range6"]
				),
				_("{range6}-Above").format(range6=cint(self.filters["range6"]) + 1),
			]
		):
